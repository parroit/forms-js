...
function bindArray(element, array) {
    ...
    function removeItemElement(removingElm) {
        //console.log('element being removeItemElement:');
        //console.log(removingElm.outerHTML);
        //console.log('---------------------------------');

try {
        if (bindedChildren.length > 1) {
            var parent = removingElm.parentElement || removingElm.parentNode;
            if (parent) {
                //console.log('parent:', parent.outerHTML);
                var children = parent.children.length;
                //console.log('children:', parent.outerHTML);
                parent.removeChild(removingElm);
                //console.log('success:%s', children == parent.children.length + 1);

            } else {

                throw new Error('Cannot find parent element of '+removingElm.outerHTML);

            }

            
        } else {
            //console.log('hiding last array element');
            element = removingElm;
